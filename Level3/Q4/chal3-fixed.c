...
void encrypt_text(const char *plaintext, char *ciphertext, const int *key_pattern, int pattern_length) {
    ...
    for (i = 0; plaintext[i] != '\0'; i++) {
        ...
        if (plaintext[i] >= 'a' && plaintext[i] <= 'z') {
            ...
            ciphertext[i] = ((plaintext[i] - '0' + current_key) % 10 + 10) % 10 + '0';
        } else {
...
int main() {
    ...
    int total_tries = 0;
    while (score < WINNING_SCORE && total_tries < MAX_TRIES) {
        total_tries++;
        printf("\nCurrent Score: %d\n", score);
        printf("Enter plaintext: ");
        if (scanf("%255s", plaintext) != 1) { // Limit input to prevent buffer overflow
            printf("Invalid input. Exiting.\n");
            break;
        }
        printf("Enter ciphertext: ");
        if (scanf("%255s", user_ciphertext) != 1) { // Limit input to prevent buffer overflow
            printf("Invalid input. Exiting.\n");
            break;
        }
